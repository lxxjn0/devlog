---
title: "Java의 문자열 파해치기"
date: 2020-02-08
description: String of Java
tags:
  - Java
  - String
slug: "/string-of-java"
...
![string heap area 01](./string-heap-01.png)
...
![string heap area 02](./string-heap-02.png)
...
![string equals test code](./string-test-code.png)

![string equals test result](./string-test-result.png)

코드를 직접 실행한 결과, `equals()`메서드는 true를 반환하고 `==`는 false를 반환하여 테스트에 통과하지 못하였다.

Java String의 `equals()`메서드 코드를 살펴보면 아래와 같다.

![string equals](./string-equals.png)
...
![string intern test](./string-intern-test.png)
...
자바 소스 파일(.java)이 클래스 파일(.class)로 컴파일되고 **JVM(*Java Virtual Machine*)**에 올라갈 때, **JVM은 String Constant Pool에 동일한 문자열이 있는지 확인**하고 이미 존재한다면 재사용을 하고 없는 경우 새로운 문자열을 만든다.

그렇기 때문에 여러 레퍼런스가 같은 문자열 리터럴을 참조하더라도 영향이 없어야하고 문자열 리터럴은 **불변(*immutable*)**하기 때문에 **thread-safe**하다.
...
> **가비지 컬렉터(*Garbage Collector*)**는 간단하게 레퍼런스를 잃어서 더 이상 사용될 수 없는 객체를 효과적으로 처리하여 메모리의 공간을 관리하는 작업을 하는 프로그램을 말한다.

하지만 Java의 String은 성능의 최적화를 위해 문자열 연산의 경우 내부적으로 **StringBuilder**를 사용하는데, 이는 다음에 추가적으로 살펴보도록 하겠다.
...
> 그래서 결론은 **문자열 리터럴**을 사용하고 new 연산자를 통한 생성은 되도록 **지양**하도록 하자!
